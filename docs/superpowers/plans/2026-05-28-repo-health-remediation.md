...
- **Branch/worktree:**
  - Branch: `feature/pr-12-performance-budgets-inline` (merged; removed)
  - Worktree: `.worktrees/pr-12-performance-budgets-inline` (removed)
- **Merged:** 2026-06-02
- **Merge commit:** `bed3084864f2eca5cf3e055cf086154c38a4e98d`
- **Depends on:** PR 9–11
...
- **PR URL:** https://github.com/tomazb/kubernetes-truenas-democratic-tool/pull/71
- **Acceptance criteria:**
...
- Added repository-wide testing strategy artifacts:
  ...
  - `docs/testing/agent-test-execution.md`
- Added matrix orchestration targets:
  - `make test-ci-gate`
  - `make test-staging`
  - `make test-release-matrix`
  - `make test-matrix`
- Added staged/deep matrix test baselines:
  - `python/tests/staging/test_staging_preflight.py`
  - `python/tests/regression/test_resilience_matrix.py`
  - `python/tests/regression/test_security_matrix.py`
- Verification evidence captured with local runs:
  - `make security-scan` (pass)
  - `make test-matrix` (pass; staging lane skipped without `TEST_STAGING=true`)
  - `artifacts/summary.json` generated by matrix target
