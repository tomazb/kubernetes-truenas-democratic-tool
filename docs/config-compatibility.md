...
| In-cluster mode | `kubernetes.in_cluster` | Not used (library uses kubeconfig or in-cluster client defaults) |
...
| TrueNAS timeout | `truenas.timeout` as duration string (`30s`) | `truenas.timeout` as integer seconds |
...
Both runtimes expand `${VAR}` and `${VAR:default}` in config files when loading from disk.
